...
const buildSkillsWithUseInMonths = (
  ...
  stages.forEach((stage) => {
    stage.stageSkills.forEach((stageSkillId) => {
      ...
      result[skill.id].useInMonths += dateDifferenceInMonth(
        stage.startedAt,
  ...
  const decoratedSkills = Object.values(result).map(
    (skillBuildObject: any) =>
      new SkillDecorator(skillBuildObject.skill, skillBuildObject.useInMonths)
  )
